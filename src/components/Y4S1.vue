      ...
      <div v-if="pending1.length > 0">
        ...
        <transition-group name="todo-item" tag="ul" class="todo-list">
          <li v-for="item1 in pending1" v-bind:key="item1.title">
            <label v-bind:for="'item_9' + item1.id"></label>
...
export default {
  ...
  watch: {
    imptList: {
      handler: function(updatedList1) {
        localStorage.setItem("y4s1_list", JSON.stringify(updatedList1));
      },
      deep: true
    }
  },
  ...
  methods: {
    ...
    getImpt() {
      if (localStorage.getItem("y4s1_list")) {
        this.imptList = JSON.parse(localStorage.getItem("y4s1_list"));
      }
    },
    // add a new item
    addItem() {
