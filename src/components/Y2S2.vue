      ...
      <div v-if="pending1.length > 0">
        ...
        <transition-group name="todo-item" tag="ul" class="todo-list">
          <li v-for="item1 in pending1" v-bind:key="item1.title">
            <label v-bind:for="'item_6' + item1.id"></label>
...
export default {
  ...
  data() {
    return {
      ...
      selected: null,
      mods: [
        "Шаблоны программирования платформ на языке Джава",
        "Иностранный язык",
        ...
        "Физическая культура",
        "Анализ и концептальное моделирование систем"
      ]
    };
  },
  mounted() {
  ...
  watch: {
    imptList: {
      handler: function(updatedList1) {
        localStorage.setItem("y2s2_list", JSON.stringify(updatedList1));
      },
      deep: true
    }
  },
  ...
  methods: {
    ...
    getImpt() {
      if (localStorage.getItem("y2s2_list")) {
        this.imptList = JSON.parse(localStorage.getItem("y2s2_list"));
      }
    },
    // add a new item
    addItem() {
